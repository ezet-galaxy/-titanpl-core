...
try {
    ...
    else if (typeof _G.t !== 'undefined') t = _G.t;
    else if (typeof _G.Titan !== 'undefined') t = _G.Titan;

    // Polyfill global aliases if found
    if (t) {
        if (typeof _G.t === 'undefined') _G.t = t;
        if (typeof _G.Titan === 'undefined') _G.Titan = t;
        if (typeof Titan === 'undefined' && typeof window === 'undefined') _G.Titan = t;
    }
} catch (e) { }
...
const url = {
    parse: (s) => new URL(s),
    SearchParams: (typeof URLSearchParams !== 'undefined' ? URLSearchParams : class { constructor(i) { this.q = i || ""; } toString() { return String(this.q); } })
};
...
if (t) {
    ...
    // 1. Force Define Properties (Reflect is robust)
    if (!t[EXT_KEY]) t[EXT_KEY] = {};
    Object.keys(API).forEach(key => {
        if (t[EXT_KEY][key] === API[key]) return;

        const success = Reflect.defineProperty(t[EXT_KEY], key, {
            value: API[key],
            writable: true,
            enumerable: true,
            configurable: true
        ...
        if (!success) {
            try { t[EXT_KEY][key] = API[key]; } catch (e) { }
        }
    });

    // 2. Prototype Injection (The Ultimate Fallback)
    try {
    ...
    // We aggressively attach properties to t for convenience.
    Object.keys(API).forEach(key => {
        ...
        try {
            ...
            // If strict mode or read-only, try defineProperty
            try { Object.defineProperty(t, key, { value: API[key], writable: true, configurable: true, enumerable: true }); } catch (i) { }
        }
    });
...
// 5. Exports
// We export the API objects so they can be imported via ESM
